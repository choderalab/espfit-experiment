...
def calc_metric(net, ds, category, suffix):
    """
    """
    u, u_ref = [], []
    u_prime, u_ref_prime = [], []
    ...
    df.to_csv(f'rmse_{suffix}_{category}.csv', sep='\t')
    PandasTools.AddMoleculeColumnToFrame(df, "SMILES", "MOL")
    open(f"rmse_{suffix}_{category}.html", "w").write(df.to_html())
    # Bootstrap over molecule
    ci_e, ci_f = bootstrap_mol(u_ref, u, u_ref_prime, u_prime)
    # Report summary
    ofile = f'summary_{category}.csv'
    if os.path.exists(ofile):
        wf = open(ofile, 'a')
    else:
        wf = open(ofile, 'w')
    wf.write(f">{category} ({suffix})\n")
    wf.write("----------\n")
...
def run(kwargs):
    ...
    for suffix in ['tr', 'vl', 'te']:
        if suffix == 'tr':
            ...
            ds = ds_te
            del ds_te
        calc_metric(net, ds, category, suffix)
        del ds
