...
def run(kwargs):
    ...
    #files = glob.glob(os.path.join(path_to_checkpoints, "ckpt*.pt"))
    #if len(files) == 0:
    #    raise FileNotFoundError("No checkpoint found")
    #files = sorted(files, key=lambda x: int(x.split('ckpt')[-1].split('.')[0]))
    ...
    #    epoch = (i+1) * model.checkpoint_frequency
