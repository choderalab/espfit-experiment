...
def run(kwargs):
    ...
    dataset_rna_diverse = CustomGraphDataset.load(f'{path_to_data}/rna-diverse').split([train_ratio, 1-train_ratio])[0]
    dataset_nucleoside = CustomGraphDataset.load(f'{path_to_data}/rna-nucleoside')
    print(f'Small molecule: {len(dataset_small)}')
    print(f'Peptide: {len(dataset_peptide)}')
    print(f'RNA diverse: {len(dataset_rna_diverse)}')
    print(f'Nucleoside: {len(dataset_nucleoside)}')

    # Split dataset
    ds = dataset_small
    for dataset in [ dataset_peptide, dataset_rna_diverse, dataset_nucleoside ]:
        ds += dataset
