...
def load_dataset(train_ratio, path_to_data):
    ...
    _, rnadiv_vl_te = CustomGraphDataset.load(f'{path_to_data}/rna-diverse').split([train_ratio, 1-train_ratio])
    rnadiv_vl_te.apply(regenerate_impropers, in_place=True)
    rnadiv_vl, rnadiv_te = rnadiv_vl_te.split([0.5, 0.5])
    print(f'RNADiverse: {len(rnadiv_vl)}, {len(rnadiv_te)}')
    
    # Merge dataset
    ds_vl = small_vl + peptide_vl + rnadiv_vl
    ds_te = small_te + peptide_te + rnadiv_te
